...
type SiteMapLocationsSectionProps = {
  locations: EventLocation[];
};

export default function SiteMapLocationsSection({
  locations,
}: SiteMapLocationsSectionProps) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-10">
        {locations.map((loc) => (
          <div key={loc.name} className="group">
            ...
            <p className="section-label text-primary mb-1">{loc.capacity}</p>
                    ...
                    {modal.view === "floor-plan" && (
                      <div className="mt-8 border border-primary/15 bg-primary/5 p-4">
                        ...
                        <p className="mt-2 font-body text-sm leading-relaxed text-charcoal/70">
                          Replace this image later with the actual floor plan
                          for {modal.location.name}.
                        </p>
                      </div>
                    )}
                  </div>
