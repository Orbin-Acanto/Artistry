  ...
  return (
    <>
        ...
        <div className="max-w-8xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="prose-venue mt-5">
              Our dedicated wedding team works alongside you from the very first inquiry through every last detail of your celebration — coordinating seamlessly with Tardis Catering, our preferred vendors, and the museum&apos;s own team to ensure a flawless, magical day.
            </p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Link href="/#contact" className="btn-primary">Request a Tour</Link>
              <Link href="/vendors" className="btn-outline-dark">View Vendors</Link>
            </div>
          </div>
      ...
      <section className="bg-primary-dark py-section px-6">
        <div className="max-w-8xl mx-auto">
          <SectionHeader
            ...
            title="Four Distinct Settings for Your Day"
            centered
            light
            className="mb-14"
          />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {weddingLocations.map((loc) => (
              <div key={loc.name} className="group">
                ...
                <h3 className="font-display text-2xl text-cream mb-2">{loc.name}</h3>
                <p className="font-body text-sm text-cream/65 leading-relaxed">{loc.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-8xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {weddingPackages.map((pkg) => (
              <div key={pkg.name} className="border border-primary/20 p-8 hover:border-primary/60 transition-colors">
                <h3 className="font-display text-2xl text-charcoal mb-3">{pkg.name}</h3>
                <p className="font-body text-sm text-charcoal/65 leading-relaxed mb-6">{pkg.description}</p>
                ...
                <Link href="/#contact" className="btn-primary mt-8 w-full text-center">
                  Inquire
                </Link>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {weddingEnhancements.map((enh) => (
              <div key={enh.name} className="bg-cream p-6">
                <h4 className="font-display text-lg text-charcoal mb-2">{enh.name}</h4>
                <p className="font-body text-sm text-charcoal/65 leading-relaxed">{enh.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-primary py-16 px-6 text-center">
        ...
        <h2 className="font-display text-display-md text-cream mb-6">Wedding Brochure</h2>
        <p className="font-body text-cream/65 text-sm max-w-md mx-auto mb-8">
          Download our complete wedding brochure for full details on spaces, packages, and the Artistry experience.
        </p>
