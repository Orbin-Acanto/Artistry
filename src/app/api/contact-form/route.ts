  ...
  try {
    ...
    if (!allowed) {
      return NextResponse.json(
        { success: false, message: 'Too many submissions from your network. Please try again tomorrow.' },
        { status: 429 }
      );
    }
    ...
    if (!startedAt || Date.now() - startedAt < MIN_FORM_TIME) {
      return NextResponse.json(
        { success: false, message: 'Form submitted too quickly. Please try again.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: true, message: 'Form submitted successfully!', data: n8nData },
      { status: 200 }
    );
  } catch (error) {
